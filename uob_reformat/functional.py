...
class Convert2File:
    ...
    async def generate_tmp_file(self) -> None:

        self.clear_tmp_file()

        logging.info("Generate data to tmp file")
        ...
        for i, record in enumerate(self.logging):
            try:
                ...
                record.update({"function": "generate_tmp_file", "status": status})
    ...
    async def generate_target_file(self) -> None:
        ...
        for i, record in enumerate(self.logging):
            try:
                ...
                # Validate data change row by row
                cmp_df = self.comparing_dataframe(i, target_df, new_df)
                cdc = self.change_data_capture(i, cmp_df)

                ## Write csv file
                status = self.write_csv(i, cdc)
                record.update({"function": "generate_target_file", "status": status})
